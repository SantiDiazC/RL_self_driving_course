...
class ActorCritic(nn.Module):  # ActorCritic Class - Created by inheriting nn.Module (provided by Pytorch) Class.
    def __init__(self, obs, actions):  # constructor - initializer(__init__): Object creation and variable initialization.
        super(ActorCritic, self).__init__()  # Calling the constructor of the inherited parent Class(nn.Module).
        self.fc1 = nn.Linear(obs, 256)  # Fully Connected: input   4 --> output 256
        self.fc_pi = nn.Linear(256, actions)  # Fully Connected: input 256 --> output   2
        self.fc_v1 = nn.Linear(256, 1)
        self.fc_v2 = nn.Linear(256, 1) # Fully Connected: input 256 --> output   1
...
def train(global_model, rank, env_name):  # Called by 3(n_train_processes) agents independently.
    ...
    for n_epi in range(max_train_ep):  # max_train_ep = 300
        ...
        while not done:  # CartPole-v1 forced to terminates at 500 steps.
            ...
            for t in range(
                    update_interval):  # Collect data during 5(update_interval) steps and proceed with training.
            ...
            R = 0.0 if done else np.minimum(local_model.v1(s_final).item(), local_model.v2(s_final).item())  # .item() is to change tensor to python float type.
            td_target_lst = []
            ...
            advantage = td_target_batch - torch.min(local_model.v1(s_batch), local_model.v2(s_batch))
            ...
            loss = -torch.log(pi_a) * advantage.detach() + F.smooth_l1_loss(local_model.v1(s_batch),
                                                                            td_target_batch.detach())\
                                                          + F.smooth_l1_loss(local_model.v2(s_batch),
...
def test(global_model, run, env_name):
    ...
    env.close()
    np.save("./rwd/" + alg + "/reward_" + env_name + "_run_" + str(run) + alg + ".npy", np.array(reward_means))
