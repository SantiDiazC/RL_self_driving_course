...
class ActorCritic(nn.Module):  # ActorCritic Class - Created by inheriting nn.Module (provided by Pytorch) Class.
    def __init__(self):  # constructor - initializer(__init__): Object creation and variable initialization.
        super(ActorCritic, self).__init__()  # Calling the constructor of the inherited parent Class(nn.Module).
        self.fc1 = nn.Linear(4, 256)  # Fully Connected: input   4 --> output 256
        self.fc_pi = nn.Linear(256, 2)  # Fully Connected: input 256 --> output   2
...
def train(global_model, rank, config):  # Called by 3(n_train_processes) agents independently.
    ...
    memory = ReplayBuffer2(config)

    optimizer = optim.Adam(global_model.parameters(),
                           lr=config["lr"])  # The optimizer updates the parameters of 'global_model'.

    env = gym.make(config["env"])  # Create 'CartPole-v1' environment.
    update_interval = config["update_interval"]
    gamma = config["gamma"]
    for n_epi in range(config["max_train_ep"]):  # max_train_ep = 300
        ...
        while not done:  # CartPole-v1 forced to terminates at 500 steps.
            ...
            for t in range(update_interval):  # Collect data during 5(update_interval) steps and proceed with training.
                ...
                m = Categorical(prob)  # Categorical probability distribution model
...
def main(config):
    ...
    for rank in range(config["n_train_processes"] + 1):  # + 1 for test process
        if rank == 0:
            p = mp.Process(target=test, args=(global_model, config,))  # Create a test processor.
        else:
            p = mp.Process(target=train, args=(global_model, rank, config,))  # Create training processor.
...
if __name__ == '__main__':
    ...
    # Hyper-parameters
    n_train_processes = config_dict['n_train_processes']  # number of independent agents. (for training)
    update_interval = config_dict['update_interval']  # Collect data during 'update_interval steps' and proceed with training.
    max_train_ep = config_dict['max_train_ep']  # max episode for training.
    max_test_ep = config_dict['max_test_ep']
    lr = config_dict['lr']
    gamma = config_dict['gamma']
    buffer_limit = config_dict['buffer_limit']  # buffer max size
    initial_exp = config_dict['initial_exp']  # TBD
    n_train_processes = config_dict['n_train_processes']
    # W&B run
    env = config_dict["env"]
    ...
    analysis = tune.run(
        ...
        config={
            ...
            "lr": lr,
            'alg': alg,
            "update_interval": tune.grid_search(update_interval),
            ...
            "n_train_processes": tune.grid_search(n_train_processes),
